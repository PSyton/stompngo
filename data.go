...
type Stomper interface {
	...
	Subscribe(headers Headers) (<-chan Message, error)
...
type Connection struct {
	...
	DisconnectReceipt Message        // If receipt requested on DISCONNECT.
	Messages          <-chan Message // Inbound data for the client.
	Errors            <-chan error   // Client errors channel
	connected         int32
	session           string
	protocol          string
	input             chan Message
...
type subscription struct {
	messages chan Message // Subscription specific Message channel
